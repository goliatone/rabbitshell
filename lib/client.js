...
Local.init = function(){
    ascoltatori.build(Local.config.amqp, function (_, ascoltatore) {
       console.log('===> AMQP client CONNECTED');
       console.log('AMQP config', JSON.stringify(Local.config.amqp, null, 4));

       var term;
       var createTopic = getTopic(Local.config.instanceId, 'create');
       ...
       ascoltatore.subscribe(createTopic, function(t, options){
        ...
        //    term = pty.spawn('/bin/bash', ['source ~/.bash_profile'], options.options);
           term = pty.spawn(options.file, [Local.config.instanceId], options.options);

           term.on('data', function(data) {
               var topic = getTopic(Local.config.serviceId, 'data');
               ascoltatore.publish(topic, data);
           });

           term.on('exit', function(data){
               var topic = getTopic(Local.config.serviceId, 'exit');
               console.log('on term.exit:', topic);
               ascoltatore.publish(topic, data);
           });

           term.on('close', function(data){
               var topic = getTopic(Local.config.serviceId, 'close');
           ...
           var topic = getTopic(Local.config.instanceId, 'resize');
           ...
           topic = getTopic(Local.config.instanceId, 'write');
           console.log('client subscribe to:', topic);
           ascoltatore.subscribe(topic, function(t, data){
               console.log('we got message on:', t);
           ...
           topic = getTopic(Local.config.instanceId, 'end');
           console.log('client subscribe to:', topic);
           ascoltatore.subscribe(topic, function(t, data){
               console.log('we got message on:', t);
               term.end();
   ...
   function getTopic(id, type){
       return Local.config.baseTopic + '/' + id + '/' + type;
   }
};
